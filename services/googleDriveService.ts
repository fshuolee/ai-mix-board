...
/**
 * Non-destructive auto-healing for image nodes that lost their Drive links.
 * Matches nodes by originalFileName against existing Google Drive files.
 */
export async function healProjectImageNodes(
  token: string,
  projectFolderId?: string,
  spreadsheetId?: string,
  assetsFolderId?: string,
  nodes: CanvasNode[] = []
): Promise<{ healedNodes: CanvasNode[]; healedCount: number }> {
  const unlinkedImageNodes = nodes.filter(n => {
    if (n.type !== 'image') return false;
    const img = n as ImageNode;
    return !img.driveFileId || !isDriveFileId(img.driveFileId) || img.driveFileId === img.id;
  });

  if (unlinkedImageNodes.length === 0) {
    return { healedNodes: nodes, healedCount: 0 };
  }
  ...
  try {
    ...
    allDriveImages.forEach(f => {
      ...
      if (m) {
        byShortId.set(m[1].slice(0, 8), f);
      }
    });

    let healedCount = 0;
    const healedNodes = nodes.map(n => {
      if (n.type !== 'image') return n;
      const img = n as ImageNode;
      const isUnlinked = !img.driveFileId || !isDriveFileId(img.driveFileId) || img.driveFileId === img.id;
      if (!isUnlinked) return n;
      ...
      if (!matched && fn.includes('Drive快取檔案')) {
        ...
        if (m) {
          matched = byShortId.get(m[1].slice(0, 8));
        }
      }
      ...
      if (matched) {
        ...
        return {
          ...
          driveViewLink: matched.webViewLink || `https://drive.google.com/file/d/${matched.id}/view`,
          status: 'idle',
          updatedAt: Date.now(),
        } as ImageNode;
      }
    ...
    return { healedNodes, healedCount };
  } catch (err) {
    console.warn('Error during healProjectImageNodes:', err);
    return { healedNodes: nodes, healedCount: 0 };
  }
}
